  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <SectionHeader title="Gestión de Cobranzas" subtitle="Control de cartera y antigüedad." />
        <div className="flex bg-white p-1.5 rounded-xl shadow-sm border border-slate-100">
          {['total', 'CLIENTE', 'PERSONAL', 'GARANTIA'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <KPI title="Cartera Total" value={`S/ ${totals.total.toLocaleString()}`} colorTheme="indigo" />
        <KPI title="Deuda Vencida" value={`S/ ${totals.vencida.toLocaleString()}`} colorTheme="rose" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card hover={true} className="lg:col-span-1 flex flex-col justify-center">
          <h3 className="font-bold text-slate-800 mb-6 text-center">Distribución de Antigüedad</h3>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.map((row, i) => (
                  <tr key={i} className="group hover:bg-indigo-50/30 transition-colors">
                    <td className="p-4 font-semibold text-slate-700 group-hover:text-indigo-700">{row.entidad}</td>
